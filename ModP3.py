...
df_art, df_track, df_top200, singleparartiste = cl.cleandata()
...
def recherchenom(nom):
    ...
    if not nom_idtq.empty:
        ...
        recence = nom_idtq.sort_values(by='release_annee', ascending=False).head(3)[['name', 'release_annee']]
...
def rechercheag(annee, genre):
    ...
    tab_track['release_annee'] = tab_track['release_date'].dt.strftime('%Y') 
    ...
    for index, row in fusion.iterrows():
        if str(row['genres']).lower() == genre.lower() and str(row['release_annee']) == str(annee):
...
#Chemin relatif pour le fichier "spotify_logo"
